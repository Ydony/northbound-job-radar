...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const job = await db.prepare(`SELECT id, source_url, source_key, source_job_id, canonical_url, identity_fingerprint,
    ...
    FROM jobs WHERE id = ?`).bind(id).first<{
      id: string;
      source_url: string;
      source_key: string;
  ...
  if (hasSavedState) statements.push(db.prepare('UPDATE jobs SET is_saved = ?, updated_at = ? WHERE id = ?')
    .bind(body.isSaved ? 1 : 0, now, id));
  if (hasApplicationStatus) statements.push(db.prepare('UPDATE jobs SET application_status = ?, updated_at = ? WHERE id = ?')
    .bind(applicationStatus, now, id));
  if (hasVisibilityStatus) {
    statements.push(db.prepare('UPDATE jobs SET visibility_status = ?, updated_at = ? WHERE id = ?')
    ...
    if (visibilityStatus === 'dismissed') {
      ...
      statements.push(db.prepare(`INSERT INTO dismissed_jobs
        ...
        .bind(id, job.source_key || source.key, sourceJobId, canonicalUrl, identityFingerprint, now));
    } else {
      statements.push(db.prepare(`DELETE FROM dismissed_jobs WHERE id = ?
        OR (? != '' AND source_key = ? AND source_job_id = ?)
        OR (? != '' AND canonical_url = ?)
        OR (? != '' AND identity_fingerprint = ?)`)
        .bind(id, job.source_job_id, job.source_key, job.source_job_id, job.canonical_url, job.canonical_url,
