...
| Manually triggered multi-source collection | Dashboard search buttons; local CLI for Indeed | `app/api/scrape/route.ts`, `lib/job-adapters.ts`, `lib/eures.ts`, `lib/job-room.ts`, `lib/freehire.ts`, `lib/ats-feeds.ts`, `lib/job-aggregators.ts`, `lib/jobsch.ts`, `lib/indeed/*`, `scripts/indeed*.mjs` | `jobs`, `search_runs`, `search_run_sources`, `indeed_control`, `indeed_coverage`; adapter/Indeed tests |
| Bounded scheduled public refresh (INT-06; Search still fans out until INT-05 rewires it) | Prod Cron Trigger (config only; deploy is owner-supervised) | `worker/entry.ts` (scheduled handler), `lib/public-refresh.ts`, `lib/public-refresh-scheduler.ts`, `vite.config.ts` triggers | `public_refresh_state`, `public_refresh_queue`; `tests/public-refresh.test.ts` |
