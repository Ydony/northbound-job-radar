...
Both use local D1 emulation with separate storage. Jobs, accounts, and search history remain
on this computer under ignored `.wrangler/` state. In-project recovery snapshots were
scrubbed of CV data on 2026-09-23; external copies were not inventoried.
...
2. Create an independent remote D1 resource; never point a deployment at local test data.
...
6. Complete a privacy review for account and behavioural data.
