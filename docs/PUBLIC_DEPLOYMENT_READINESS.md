...
Assessment from the code on 2026-09-23, updated 2026-09-24. The owner approved a
separate, private, single-admin Cloudflare production plan (milestone 12; initial
D1 configuration in commit `35cfa9e`). It is now live: the Worker, its remote D1,
owner-only secrets, and the sole administrator account are configured, an
authenticated sign-in returned HTTP 200 on 2026-09-24, and `https://ikbeneenappel.nl`
serves the app over HTTPS with the correct security headers (verified
independently the same day; `www` does not redirect yet). DEV (`:3000`) and TEST
(`:3001`) remain local; do **not** publish either one or copy their D1 state or
credentials to the host. The blockers below govern a future *public/open-registration*
service — they were never satisfied by the private, closed-registration deployment,
and still are not; a single-admin installation with registration permanently
closed does not need them. “Administrator-only” is a code boundary, not permission
from a job source.
...
| Accepted gap | **Owner decision 2026-09-24: no data-export feature, by design, not oversight.** `lib/export.ts` and its test were deleted (it was untested-in-product dead code after the 2026-09-22 UI removal). Users can delete their account/data (see the deletion-completeness row below) but cannot download it first. NL users have a GDPR Article 20 portability right this does not satisfy; the owner was told this explicitly and accepted the gap rather than ship export. | If this is ever revisited, re-read this row first — it was a deliberate choice, not a TODO. Define retention/deletion and complete a privacy review regardless; those don't require export to exist. |
| Blocker | The private installation's remote D1, Worker, owner-only secrets and sole administrator are now configured and verified working (2026-09-24). That satisfies a single-admin, closed-registration deployment; it does not make a public service ready. | Before public traffic, verify isolated production credentials, deployment permissions, migrations, rollback and abuse controls at public scale. Never use local TEST data as production seed. |
