...
- share of everything found that was **added** at all
- share that is **English-confirmed**
- share **filtered out** because a local language is required
- share that is **unknown** (the ad was too short to judge)
...
| Filter | Change |
|---|---|
| Role keywords | **3 shown by default, a `+` button to add up to 10.** Currently capped at 5. |
...
- Seniority.
- Contract type.
- **Location.** The owner reversed on this mid-sentence and settled on removing it: the location is
  shown on each job card, so people can see where a job is without filtering on it beforehand.
...
- [ ] Raise `MAX_ROLE_KEYWORDS` from 5 to 10 and add the `+` control, 3 visible initially.
...
| B1/B2 | No password reset, no email verification | An email provider — **only mandatory if registration opens to others**; registration is closed today |
